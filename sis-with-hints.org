...
In the jolly tradition of [[https://malb.io/are-graded-encoding-schemes-broken-yet.html]["Are Graded Encoding Schemes broken yet?"]]  I tag problems as BROKEN if I am aware of cryptanalysis. I also tag them as:
...
#+begin_quote
For any integer \(k \geq 0\), an instance of the k-M-SIS problem is a matrix \(\mat{A} \in \ring_{q}^{n \times m}\) and a set of \(k\) vectors \(\vec{u}_{0}, \ldots \vec{u}_{k-1}\) s.t. \(\mat{A}\cdot \vec{u}_{i} \equiv \vec{0} \bmod q\) with \(\|{\vec{u}_i}\| \leq \beta\). A solution to the problem is a nonzero vector \(\vec{u} \in \ring^{m}\) such that
...
The original paper showed that k-SIS (over \(\ZZ\)) is hard if SIS is hard for uniform \(\mat{A}\) and discrete Gaussian \(\vec{u}_{i}\). This reduction was improved in [cite:@C:LPSS14] to cover \(k = \mathcal{O}(m)\).

The condition \(\vec{u} \notin \mathcal{K}\text{-}\operatorname{span}(\set{\vec{u}_i}_{0 \leq i < k})\) can be dropped when \(k < m^{1/4}\) because then the probability that there is an additional short vector in the \(k\)-dimensional sublattice spanned by the \(\vec{u}_i\) is negligible.
...
#+begin_quote
A challenger selects a matrix \(\mat{A} \in \ZZ_{q}^{n \times m}\) and sends it to the adversary. The adversary can perform two types of queries:

*Syndrome queries* The adversary can request a challenge vector which the challenger selects at random, i.e. \(\vec{t} \gets_{\$} \ZZ_{q}^{n}\), adds to some set \(\mathcal{S}\) and returns to the adversary.

*Preimage queries.* The adversary submits /any/ vector \(\vec{t}' \in \ZZ_{q}^{n}\). The challenger will return a short vector \(\vec{y}' \gets_{\$} D_{\ZZ^m,\sigma}\) such that \(\mat{A}\cdot \vec{y}' \equiv \vec{t}' \bmod q\). Denote \(k\) for the number of preimage queries.
...
*Combinatorial Attack.* The adversary requests \(n \cdot q\) preimages for all \(\{a \cdot \vec{e}_{i}\ \mid\ a \in \ZZ_{q}, i \in \ZZ_{n}\}\), here \(\vec{e}_{i}\) is the \(i\)-th unit vector. Then, adding up \(n\) such preimages allows to construct any image. Since the norm of the preimages returned by the challenger is \(\sqrt{m} \cdot \sigma\), this allows to solve the One-more-ISIS problem when \(\sqrt{n \cdot m} \cdot \sigma \leq \beta\). Of course, smaller and larger sets of preimages are possible, increasing and decreasing the output norm respectively.

*Lattice Attack.* The adversary requests \(\geq m\) preimages of zero and uses that to produce a short basis \(\mat{T}\) for the kernel of \(\mat{A}\), i.e. \(\mat{A}\cdot\mat{T} \equiv \vec{0} \bmod q\). This constitutes a trapdoor for \(\mat{A}\) and thus permits to return short preimages for any target. The key point here is that this trapdoor is of degraded quality relative to the trapdoor used by the challenger. The key computational challenge then is to fix-up or improve this degraded trapdoor in order to be able to sample sufficiently short vectors.

** Variants

In [cite:@AC:BCGY24] a variant of OM-ISIS is introduced, referred to as "randomised OM-ISIS" by the authors.[fn::The randomisation in this variant is not different from OM-ISIS.] This variant imposes additional constraints on the preimages \(\vec{y}_{i}\) in that some components are required to be in \(\{-1,1\}\). In particular, the preimages (both those output by the challenger and the adversary) are split into \(\vec{y}_{i,0}\) and \(\vec{y}_{i,1}\) where \(\Vert\vec{y}_{i,0}\Vert \leq \beta\) as above and \(y' \in \{-1,1\}\) for all entries \(y'\) of \(\vec{y}_{i,1}\). 
...
Let  \(\mat{A} \gets_{\$} \ring_q^{n \times m}\), \(\vec{v} \gets_{\$} (\ring_q^\star)^w\). The /K-M-ISIS/ assumption states that given \((\mat{A}, \vec{v}, \vec{t}, \{\vec{u}_{g}\})\) with \(\vec{u}_{g}\) short and
\[
...
\]
When \(n = 1\), i.e. when \(\mat{A}\) is just a vector, we call the problem /k-R-ISIS/.
#+end_quote
...
- k-R-ISIS (with \(g^{*} \equiv 1\)) is as hard as R-SIS when \(m > k\) or when the system generated by \(\mathcal{G}\) is efficiently invertible.
...
The knowledge assumption only makes sense for \(n \geq 2\). However, in order to be able to pun about "crises of knowledge", the authors also define a ring version of the knowledge assumption. In the ring setting, they consider proper ideals rather than submodules.
...
Let \(\mat{A} \gets_{\$} \ring_q^{n \times m}\), \(\mat{B} \gets_{\$} \ring_q^{n \times m}\), \(\vec{v} \gets_{\$} (\ring_q^\star)^w\). The /Twin K-M-ISIS/ assumption states that given \((\mat{A}, \mat{B}, \{\vec{u}_{g}\}_{g \in \mathcal{G}_A}, \{\vec{w}_{g}\}_{g \in \mathcal{G}_B}, \vec{t}, \vec{v})\) with \(\vec{u}_{g}, \mathbf{w}_{g}\) short,
\[
...
\[\vec{B} := \begin{pmatrix}\mat{A}_{0} & \vec{0} & - \vec{G}_{n+1}\\
\vec{0} & \mat{A}_{1} & -\vec{G}_{n+1}\end{pmatrix}\] and a short \(\vec{T}\) s.t.
\[\vec{G}_{n'} \equiv \vec{B} \cdot \vec{T} \bmod q\]
...
A way of looking at is by noting that for each column \(\vec{t} = (\vec{t}^{(0)}, \vec{t}^{(1)}, \vec{t}^{(G)})\) of \(\vec{T}\) we have \(\mat{A}_{i} \cdot \vec{t}^{(i)} \equiv \vec{G}_{n+1} \cdot \vec{t}^{(G)}\) where \(\vec{G}_{n+1} \cdot \vec{t}^{(G)}\) is close to uniform. Thus, we can sample \(\vec{t}^{(0)}\), compute \(\vec{y} := \mat{A}_{0} \cdot \vec{t}^{(0)}\) and then use the gadget structure of \(\vec{G}_{n+1}\) to find a short \(\vec{t}^{(G)}\) s.t. \(\mat{A}_{i} \cdot \vec{t}^{(i)} \equiv \vec{G}_{n+1} \cdot \vec{t}^{(G)}\). Then, using the trapdoors for \(\mat{A}_{i}\) with \(i>0\) we can find \(\vec{t}^{(i)}\) s.t. \(\mat{A}_{i} \cdot \vec{t}^{(i)} \equiv \vec{G}_{n+1} \cdot \vec{t}^{(G)}\).
...
\vec{0} & \mat{A}_{1} & -\vec{G}_{n+1}\end{pmatrix}\] and a short \(\vec{T}\) s.t.
\[\vec{G}_{n'} \equiv \vec{B} \cdot \vec{T} \bmod q\]
where \(\mat{A}_{i} := \vec{W}_{i} \cdot \mat{A}\) for known \(\vec{W}_{i} \in \ZZ_{q}^{n \times n}\).
...
The authors also show that given an algorithm for solving BASIS\(_{struct}\) there is an algorithm for solving k-M-ISIS. This reduction outputs a BASIS\(_{struct}\) instance of size \(k < m/k'\) where \(k'\) is the parameter \(k\) of the k-M-ISIS instance.
...
\[\vec{B} := \begin{pmatrix}
\mat{A} &               \vec{0} & \cdots & - \vec{G}_{n+1}\\
\vec{0} &           w \cdot \mat{A} & \cdots & -\vec{G}_{n+1}\\
\mat{0} &               \vec{0} & \ddots & -\vec{G}_{n+1}\\
\vec{0} & \cdots & w^{m-1} \cdot \mat{A} & -\vec{G}_{n+1}
...
If \(m=2\) then PRISIS is no easier than M-SIS [cite:@EPRINT:FenNgu23].
...
#+begin_quote
Let \(\mat{A}_{i} \in \ring_{q}^{n \times m}\) for \(i \in \{0,…,h-1\}\). We're given
\[\vec{B}_{i} := \begin{pmatrix}
\mat{A}_{i} &               \vec{0} & \cdots & - \vec{G}_{n+1}\\
\vec{0} &           w_{i} \cdot \mat{A}_{i} & \cdots & -\vec{G}_{n+1}\\
\mat{0} &               \vec{0} & \ddots & -\vec{G}_{n+1}\\
\vec{0} & \cdots & w_i^{m-1} \cdot \mat{A}_{i} & -\vec{G}_{n+1}
\end{pmatrix}\] and a short \(\vec{T}_{i}\) s.t.
\[\vec{G}_{n'} \equiv \vec{B}_{i} \cdot \vec{T}_{i} \bmod q.\]
...
\(h\)-PRISIS is no easier than PRISIS [cite:@EPRINT:AFLN23]. In particular, if \(m=2\) then \(h\)-PRISIS is no easier than M-SIS [cite:@EPRINT:AFLN23].
...
#+begin_quote
Let \(\mat{A} \in \ZZ_{q}^{n \times m}\) and \(\mat{W} \in \ZZ_{q}^{\ell n \times m}\). We're given \(\mat{B} := (\mat{I}_\ell \otimes \mat{A} \mid \mat{W})\) and a short \(\mat{T}\) s.t.
\[\mat{I}_\ell \otimes \mat{G} = \mat{B}\mat{T} \bmod q.\]

The \(\ell\)-succinct SIS assumption states that given \((\mat{A}, \mat{W}, \mat{T})\) it is hard to find a short \(\vec{x}\) s.t. \(\mat{A} \cdot \vec{x} \equiv \vec{0}\).
#+end_quote

** Hardness

In [cite:@EPRINT:Wee24a], it is shown that BASIS\(_{struct}\) with parameter \(k\) implies \(\ell\)-succinct SIS with parameter \(\ell=k\).

Furthermore, we can observe that SIS implies \(1\)-succinct SIS: the reduction samples \(\mat{W}\sample \ZZ_{q}^{n \times m}\) along with a trapdoor, which is used to sample a trapdoor for \((\mat{A} \mid \mat{W})\).
...
#+begin_quote
Let \(\mat{A} \in \ZZ_{q}^{n \times m}\) and let \(f: \ZZ_{N} \rightarrow \ZZ_{q}^{n}\) be a function. Given \((\mat{A}, f)\) and access to an oracle that when called samples a fresh \(x \sample \ZZ_{N}\) and outputs \(x, \vec{u}_{x}\) s.t. \[

...
- *Cryptanalysts:* please consider analysing these assumptions.

* k-SIS

[cite:@PKC:BonFre11]

We give the module variant defined in [cite:@C:ACLMT22].
...
The original paper showed that k-SIS (over \(\ZZ\)) is hard if SIS is hard for uniform \(\mat{A}\) and discrete Gaussian \(\vec{u}_{i}\). This reduction was improved in [cite:@C:LPSS14] to cover \(k = \mathcal{O}(\ell)\). No proof was provided for the module variant.
...
*Preimage queries.* The adversary submits /any/ vector \(\vec{t}' \in \ZZ_{q}^{n}\). The challenger will return a short vector \(\vec{y}' \gets_{\$} D_{\ZZ^m,\sigma}\) such that \(\mat{A}\cdot \vec{y}' \equiv \vec{t}' \bmod q\). Denote \(\ell\) for the number of preimage queries.

In the end the adversary is asked to output \(\ell+1\) pairs \(\{(\vec{y}_i,\vec{t}_i)\}_{0 \leq i < \ell+1}\) satisfying:
...
*Combinatorial Attack.* The adversary requests \(n \cdot q\) preimages for all \(\{a \cdot \vec{e}_{i} | a \in \ZZ_{q}, i \in \ZZ_{n}\}\), here \(\vec{e}_{i}\) is the \(i\)-th unit vector. Then, adding up \(n\) such preimages allows to construct any image. Since the norm of the preimages returned by the challenger is \(\sqrt{m} \cdot \sigma\), this allows to solve the One-more-ISIS problem when \(\sqrt{n\cdot m} \cdot \sigma \leq \beta\). Of course, smaller and larger sets of preimages are possible, increasing and decreasing the output norm respectively.

*Lattice Attack.* The adversary requests \(> m\) preimages of zero and uses that to produce a short basis \(\mat{T}\) for the kernel of \(\mat{A}\), i.e. \(\mat{A}\cdot\mat{T} \equiv \vec{0} \bmod q\). This constitutes a trapdoor for \(\mat{A}\) and thus permits to return short preimages for any target. The key point here is that this trapdoor is of degraded quality relative to the trapdoor used by the challenger. The key computational challenge then is to fix-up or improve this degraded trapdoor in order to be able to sample sufficiently short vectors.
...
Let  \(\mat{A} \gets_{\$} \ring_q^{\eta \times \ell}\), \(\vec{v} \gets_{\$} (\ring_q^\star)^w\).
Given \((\mat{A}, \{\vec{u}_{g}\})\) with \(\vec{u}_{g}\) short and
\[
...
The hardness of the problem is analysed by the authors by providing some partial reductions, i.e. covering some special cases. The authors show that
...
No analysis is provided to date.
